...
pub fn generate_mnemonic_from_salt(salt: &[u8]) -> Vec<String> {
    ...
    assert_eq!(salt.len(), ENTROPY_LENGTH / 8, "Salt length must be {} bytes", ENTROPY_LENGTH / 8);
...
mod tests {
    ...
    fn test_mnemonic_to_seed() {
        // Use 32 bytes of entropy (256 bits)
        let entropy = [0u8; 32];
        let mnemonic_words = generate_mnemonic_from_salt(&entropy);
        let passphrase = "password";
        
        let seed = mnemonic_to_seed(&mnemonic_words, &passphrase.to_string());
        
        assert_eq!(seed.len(), 64, "Seed should be 64 bytes");
        
        // Also verify against known test vector
        let seed_in_hex = seed
            .iter()
            .map(|b| format!("{:02x}", b))
            .collect::<Vec<String>>()
            .join("");

        assert_eq!(seed_in_hex, "6226705e713f303e6bcd9750e1996b9f5dfa6fb842ab3887e9ee9302623832be79eca3dd73fc1100da30542700a8fecd4accb62cd4e930622dcd835cf00e2c15");
    }
}
